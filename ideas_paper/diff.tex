...
%DIF DEL mixed-dim-model.tex          Fri Jun  9 08:04:28 2017
%DIF ADD mixed-dim-model-review.tex   Fri Jun  9 11:46:57 2017
...
The approach \DIFdelbegin \DIFdel{simplifies the }\DIFdelend \DIFaddbegin \DIFadd{replaces a }\DIFaddend full three-dimensional system \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend a two-dimensional overland thermal hydrology system \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend a family of one-dimensional vertical columns, where each column represents a fully coupled surface/subsurface thermal hydrology system without lateral flow. The \DIFdelbegin \DIFdel{overland thermal hydrology }\DIFdelend system is then operator split, \DIFdelbegin \DIFdel{incrementally updating lateral surface fluxes with no sources to redistribute mass and energy horizontally, followed and update including sources and sinks to couple to the columns}\DIFdelend \DIFaddbegin \DIFadd{sequentially updating the overland flow system without sources and the one-dimensional columns without lateral flows}\DIFaddend . 
We show that the approach is highly scalable, supports subcycling of different processes, and compares well with the corresponding fully three-dimensional representation at significantly less computational cost. \DIFdelbegin \DIFdel{These advances enable state-of-the-art }\DIFdelend \DIFaddbegin \DIFadd{Those advances enable recently developed }\DIFaddend representations of freezing soil physics to be coupled with thermal overland flow and surface energy balance at scales of 100s of meters.
...
\begin{keyword}
\DIFdelbegin \DIFdel{Mixed-dimensional model}\DIFdelend \DIFaddbegin \DIFadd{Multiscale models}\DIFaddend \sep Permafrost thermal hydrology  \sep Integrated surface/subsurface flow modeling \sep Arctic 
%\MSC[2010] 00-01\sep  99-00
\end{keyword}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \fntext[copyrightnotice]{ This manuscript has been authored by UT-Battelle, LLC under Contract No. DE-AC05-00OR22725 with the U.S. Department of Energy. The United States Government retains and the publisher, by accepting the article for publication, acknowledges that the United States Government retains a non-exclusive, paid-up, irrevocable, worldwide license to publish or reproduce the published form of this manuscript, or allow others to do so, for United States Government purposes. The Department of Energy will provide public access to these results of federally sponsored research in accordance with the DOE Public Access Plan (http://energy.gov/downloads/doe-public-access-plan}
%DIFDELCMD < %%%
...
Two- and three-dimensional models with explicit physics-based representations of ice/liquid/gas partitioning in the vadose zone \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{painter2014constitutive}
}%DIFAUXCMD
}\DIFdelend have only recently started to appear\DIFdelbegin \DIFdel{. Painter }%DIFDELCMD < [%%%
...
. Painter \mbox{%DIFAUXCMD
\cite{marsflo}
}%DIFAUXCMD
}\DIFaddend developed the three-phase, two-component model MarsFlo which has been used in Mars \cite{grimm2009mars} and Earth permafrost studies \cite{frampton2011}.  Karra et al. \DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{\mbox{%DIFAUXCMD
\citeyear{karra2014three}
}%DIFAUXCMD
}%DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{karra2014three}
}%DIFAUXCMD
}\DIFaddend simplified that subsurface freezing-soil thermal hydrology representation by ignoring gas advection and implemented that Richards-like approximation in the highly parallel PFLOTRAN \cite{pflotran-web-page} code. Kumar et al.~\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{\mbox{%DIFAUXCMD
\citeyear{kumar2016modeling}
}%DIFAUXCMD
}%DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{kumar2016modeling}
}%DIFAUXCMD
}\DIFaddend used that implementation in three-dimensional microtopography-resolving thermal hydrology simulations of polygonal tundra. Those computer codes are all subsurface-only models; that is, they do not represent surface flows and surface energy balance. Painter et al. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeyear{spainter2016integrated}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{spainter2016integrated}
}%DIFAUXCMD
}\DIFaddend recently introduced the Arctic Terrestrial Simulator, which uses a sophisticated multiphysics management framework \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ecoon2016managing}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{coon2016managing}
}%DIFAUXCMD
}\DIFaddend to couple the three-dimensional subsurface representation of \cite{karra2014three} with a two-dimensional non-isothermal surface flow model, surface energy balance with and without snow, and a simple snow distribution model. 
...
Custom mesh generation tools are then used to construct a 3D mesh by extruding each of the surface polygons vertically into the subsurface, \DIFdelbegin \DIFdel{introducing soil layering , etc}\DIFdelend \DIFaddbegin \DIFadd{honoring soil layering structure}\DIFaddend . 
This 3D mesh represents the entire domain of interest and is referred to as the primary mesh.
For the purpose of this model, each \DIFdelbegin \DIFdel{tesselated }\DIFdelend \DIFaddbegin \DIFadd{tessellated }\DIFaddend ice-wedge polygon, along with the volume of soil directly below it, is extracted as a single, one-dimensional (in the z-direction) \DIFdelbegin \DIFdel{, columnof soil}\DIFdelend \DIFaddbegin \DIFadd{column}\DIFaddend . 
On each column, vertical flow of mass and transport of energy are solved, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{allowing for ponding of water and accumulation of associated energy }\DIFaddend on each column's surface\DIFdelbegin \DIFdel{, a (local) }\DIFdelend \DIFaddbegin \DIFadd{. A }\DIFaddend surface balance is \DIFdelbegin \DIFdel{performed }\DIFdelend \DIFaddbegin \DIFadd{also performed on each column's surface }\DIFaddend to determine the net source of mass and energy into the system. 
\DIFdelbegin \DIFdel{These columns are then coupled through lateral surface flow, which quickly moves water and energy throughout the domain.
}\DIFdelend We note that strong (\DIFdelbegin \DIFdel{globally }\DIFdelend implicit) coupling of the \DIFdelbegin \DIFdel{surface-to-subsurface fluxes }\DIFdelend \DIFaddbegin \DIFadd{surface, subsurface, and surface energy balance }\DIFaddend are critically important for \DIFdelbegin \DIFdel{mass and energy conservation and accuracy }\DIFdelend \DIFaddbegin \DIFadd{accuracy and stability }\DIFaddend of the system \DIFdelbegin \DIFdel{through strong nonlinearities.
}\DIFdelend \DIFaddbegin \DIFadd{in the presence of strong nonlinearities; this is imposed by solving each column as an integrated system that includes a surface cell in addition to the soil below. 
The columns are then coupled through lateral surface flow, which quickly moves water and energy throughout the domain.
}
...
\caption{An illustration of the independent 1D \DIFdelbeginFL \DIFdelFL{subsurface }\DIFdelendFL columns coupled to the overland system. \DIFaddbeginFL \DIFaddFL{Although shown here as a subsurface system only, each column also includes a single surface cell with the soil column, and is coupled to a surface energy balance calculation.}\DIFaddendFL }
\label{surf-cols}
\end{figure}

\subsection{Operator Splitting Scheme}
Due to \DIFdelbegin \DIFdel{this needed }\DIFdelend \DIFaddbegin \DIFadd{the need for }\DIFaddend strong coupling between the surface and subsurface, the above mixed-dimensional strategy \DIFdelbegin \DIFdel{alone still results in a computational }\DIFdelend \DIFaddbegin \DIFadd{would, by itself, still result in a computationally }\DIFaddend expensive system of equations \DIFdelbegin \DIFdel{.
The tight coupling results in the need to solve }\DIFdelend \DIFaddbegin \DIFadd{that 
solves }\DIFaddend all the columns and the surface flow equations simultaneously.
To \DIFdelbegin \DIFdel{break this system, we develop }\DIFdelend \DIFaddbegin \DIFadd{reduce that computational burden, we developed }\DIFaddend a two-stage operator-split strategy for the surface flow and energy equations.
Under this strategy, we split the lateral fluxes from the coupling fluxes, \DIFdelbegin \DIFdel{incrementally }\DIFdelend \DIFaddbegin \DIFadd{first }\DIFaddend advancing one and then the other.
...
Then, in a second update, the coupling fluxes and columnar subsurface \DIFdelbegin \DIFdel{may be }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend solved implicitly, but independently of every other column.
...
This stage is hereafter called the ``\DIFdelbegin \DIFdel{column-surface }\DIFdelend \DIFaddbegin \DIFadd{column }\DIFaddend system.''
This splitting is shown schematically in Fig.~\ref{coupling-schematic}.
For the sake of clarity, we will refer to the pressure and temperature fields after the first stage as ``overland-flow'' pressure and temperature, while those after the second step will be called ``subsurface'' and ``column-surface'' pressures and temperatures. \DIFaddbegin \DIFadd{The splitting scheme conserves mass and energy but introduces splitting error, which is quantified through numerical experiments below. 
}\DIFaddend %
\begin{figure}[h]
\centering
...
First, and not trivially, physics and simulation data must be entirely modular and encapsulated, so that multiple instances of each physics process can be instantiated, allowing \DIFdelbegin \DIFdel{seperate }\DIFdelend \DIFaddbegin \DIFadd{separate }\DIFaddend solution on multiple domains.
Next, coupling these many processes on many domains requires a flexible, hierarchical coupling framework.
Much of this was supported by Amanzi-ATS's multiphysics library, Arcos\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ecoon2016managing}
}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{. }\newtext{A brief overview of the Arcos framework is provided in section~\ref{arcos-framework}, for more details see~\mbox{%DIFAUXCMD
\cite{coon2016managing}
}%DIFAUXCMD
}\DIFadd{. 
}\DIFaddend Arcos represents physics on these domains as a hierarchical \DIFdelbegin \DIFdel{PK }\DIFdelend \DIFaddbegin \newtext{Process Kernel} \DIFadd{(PK) }\DIFaddend tree which shows how the processes are coupled on and across these domains, as illustrated in Fig.~\ref{pk-tree}.
The PK tree consists of individual conservation equations \DIFaddbegin \newtext{(for example, the diffusion wave equation for overland flow)}\DIFaddend , strong (globally implicit) couplers, and weak (sequential) couplers highlighted in blue, light cyan, and orange colors, respectively.
In our approach, the operator splitting between the overland and column-surface systems happens at the top level weak MPC. \DIFaddbegin \newtext{As discussed earlier, an MPC is a Multiprocess Coordinator which manages coupling among PKs.}
\DIFaddend The strong MPC (on the left at the second level) is the overland system; the weak MPC at the second level iterates over all the column subdomains.
...
Not surprisingly, as the value of $\alpha$ increases the mean annual thaw depth deviates from the results of the 3D model to some extent, but we still see the results of the mixed-dimensional model agree well with the corresponding benchmark solution. The \DIFaddbegin \DIFadd{annual thaw depth is mainly determined by heat input to the subsurface during the summer season with a very limited memory effect from the previous year. Thus, we do not expect errors in the annual thaw depth to accumulate year to year.
}

\DIFadd{The }\DIFaddend consistency of our numerical results with the fully coupled 3D simulations confirm the appropriateness of \DIFdelbegin \DIFdel{this approximated scheme. }\DIFdelend \DIFaddbegin \DIFadd{the approximate scheme. As with any splitting scheme, accuracy and numerical performance will depend on the time step size. Results shown here were obtained with an adaptive time step determined by the minimum preferred time step for surface and subsurface PKs. 
}\DIFaddend 


\DIFaddbegin \newtext{Lastly, for demonstration purpose only, the surface ponded depth and temperature during the snowmelt in 2012 are presented in Fig.~\ref{barrow-pd-temp} for the 468 polygon domain. Fully coupled 3D simulations at such a scale are computationally very expensive.}


 \DIFaddend \begin{figure}[!htpb]
\centering
...
\caption{Comparison of the subsurface \DIFdelbeginFL \DIFdelFL{water }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{liquid }\DIFaddendFL saturation at locations 1 and 5 during the summer. \DIFaddbeginFL \DIFaddFL{Results correspond to $\alpha = 1$.}\DIFaddendFL }
\label{ss-sat-comp}
\end{figure}
...
\caption{Comparison of subsurface temperature at locations 1 and 5 during the summer. \DIFaddbeginFL \DIFaddFL{Results correspond to $\alpha = 1$.}\DIFaddendFL }
\label{ss-temp-comp}
\end{figure}

\begin{figure}[!htpb]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height = 7.5cm, width=6.cm]{figures/comparison/regular/ponded-depth/comp-pd-location1.eps}
%DIFDELCMD < \includegraphics[height = 7.5cm, width=6.cm]{figures/comparison/regular/ponded-depth/comp-pd-location5.eps}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height = 7.cm, width=6.cm]{figures/comparison/regular/ponded-depth/comp-pd-location1.eps}
\includegraphics[height = 7.cm, width=6.cm]{figures/comparison/regular/ponded-depth/comp-pd-location5.eps}
\DIFaddendFL \caption{An illustration of the surface ponded depths of the two schemes at locations 1 and 5 \DIFdelbeginFL \DIFdelFL{when }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{during }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{snow melt starts}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{summer}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Results correspond to $\alpha = 1$.}\DIFaddendFL }
\label{surf-pd-comp}
\end{figure}
...
\caption{An illustration of the surface temperature of the two schemes at location 1 for the entire year. %DIF < The bottom plot highlights the difference between the temperatures at location 1 and 5, though they appear quite similar in the top two plots.
\DIFaddbeginFL \DIFaddFL{Results correspond to $\alpha = 1$.}\DIFaddendFL }
\label{surf-temp-comp}
\end{figure}

%DIF < \begin{comment}
%DIF < \begin{figure}[!htpb]
%DIF < \centering
%DIF < \includegraphics[height = 6.cm, width=10.cm]{figures/comparison/regular/error.eps}
%DIF < \includegraphics[height = 6.cm, width=10.cm]{figures/comparison/dist/Elev-grad1.5m/error.eps}
%DIF < \includegraphics[height = 6.cm, width=10.cm]{figures/comparison/dist/Elev-grad2.5m/error.eps}
%DIF < \caption{The root mean square difference of the subsurface water saturation at the selected locations of the three studies.}
%DIF < \label{error-plots}
%DIF < \end{figure}
%DIF < \end{comment}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{figure}[!htpb]
\centering
...
\caption{A comparison of the annual thaw depth at the selected locations of the three studies. \DIFaddbeginFL \DIFaddFL{Results correspond to $\alpha = 1$.}\DIFaddendFL }
\label{thaw-depth}
\end{figure}
...
\caption{Simulation results \DIFdelbeginFL \DIFdelFL{of the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{from our }\DIFaddendFL mixed-dimensional model. \DIFdelbeginFL \DIFdelFL{Showing }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Shown are }\DIFaddendFL the surface ponded depth and temperature during the snowmelt of 2012. \DIFaddbeginFL \DIFaddFL{For clarity, the subsurface variables are not shown. }\DIFaddendFL }
\label{barrow-pd-temp}
\end{figure}


\subsection{Speedup Study}
We discuss speedup and parallel efficiency for two spatial domains, one with 75 polygons \DIFdelbegin \DIFdel{as depicted in }\DIFdelend \DIFaddbegin \DIFadd{(see }\DIFaddend Fig.~\ref{surf-location}\DIFaddbegin \DIFadd{) }\DIFaddend and a larger one consisting of 468 \DIFdelbegin \DIFdel{ice-wedge polygons , as shown in }\DIFdelend \DIFaddbegin \DIFadd{polygons (see }\DIFaddend Fig~\ref{barrow-468}\DIFdelbegin \DIFdel{. The surface ponded depth and temperature during the snowmelt in 2012 are presented in Fig.~\ref{barrow-pd-temp} for the 468 polygon domain. Fully coupled 3D simulations at such a scale are computationally very expensive.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{). }\DIFaddend We highlight two aspects of the efficiency of this modeling approach: (i) how the simulation time decreases in comparison with three-dimensional simulations, and (ii) how efficiently it scales with number of processes. 
...
\begin{figure}[!htpb]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height = 6.5cm, width=10cm]{figures/speedup-lcs-lobster.eps}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[height = 6.5cm, width=10cm]{figures/speedup-lcs-lobster.eps}
...
We compared our numerical results to the conventional scheme of a fully 3D subsurface that is strongly coupled to a surface system to demonstrate the efficiency and accuracy of our modeling approach. The fully coupled results act as a benchmark for our scheme. Numerical results show our scheme closely approximates the fully coupled system but is significantly more efficient. The scheme also allows for subcycling of individual subdomains, which further improves the numerical efficiency. \DIFaddbegin \DIFadd{With subcycling we see about an order of magnitude improvement in execution time compared to the fully 3D configuration on a 75-polygon mesh. However, we expect the computational advantages to improve with the system size. Indeed, we were not able to run the 468-polygon domain in fully 3D mode due to computational limitations, but were readily able to do it with the mixed-dimensional approximation. 
}\DIFaddend 

This work is part of a larger effort to provide process-rich, watershed-scale simulations capability for permafrost regions.  \DIFdelbegin \DIFdel{A next step would be }\DIFdelend \DIFaddbegin \DIFadd{Given the good scaling properties of our approach, we anticipate that simulations resolving individual ice-wedge polygons are now feasible for domains corresponding to those of a next-generation Earth system model grid cell ($\approx$ 10 km). in addition, the spatial structure used here is applicable at continental scales if a coarser discretization is used, wherein each surface grid cell corresponds to a sub-watershed in a ESM grid cell. 
}

\DIFadd{From the model development perspective, an important next step is }\DIFaddend to incorporate a subgrid model to represent the effects of  variations of topography below our discretization unit of the ice wedge polygon. Another future direction is to represent thaw-induced subsidence. Thawing of permafrost and melting of massive subsurface ice can cause differential subsidence, leading to dynamic microtopography (low-centered polygons can transform to high-centered polygons)~\cite{jorgenson2006abrupt,liljedahl2012ice}, substantial changes in hydrology and soil moisture, and altered drainage networks, thus potentially transforming a \DIFdelbegin \DIFdel{dry region to a wetland ecosystem }\DIFdelend \DIFaddbegin \DIFadd{wetland ecosystem to a dry region}\DIFaddend ~\cite{hinzman2005evidence,rowland2010arctic}. This modeling strategy is designed to tractably represent  thaw-induced subsidence. Representing subsidence in one dimension is significantly easier than a fully three-dimensional representation because mesh tangling and other mesh quality issues arise in a fully three-dimensional dynamic mesh but are avoided in one dimension. Indeed, simulations of thaw-induced subsidence on a single one-dimensional integrated surface/subsurface system has already been demonstrated \cite{painter2013modeling}; the work described here will allow the same techniques to be used at scale with many columns coupled to an overland flow system\DIFdelbegin \DIFdel{. Lastly, refactoring ATS not only supports subdomain modeling techniques but is also potentially useful future extensions. We can efficiently implement and independently test many process representations (physical, chemical, biological and geological processes) as PKs and let them interact through MPCs}\DIFdelend . 
