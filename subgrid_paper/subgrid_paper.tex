...
The role of surface microtopography is critical for surface flow but well understood. Fine-scale simulations are required to capture microtopographic influences on flow, however, these high-resolution simulations are not tractable at large spatial and temporal scales. To incorpoate the effects of surface microtopography in large-scale simulaitons, the accumulation term and flow law in the governing surface flow equations need to be altered based on the fine-scale microtopography. Simulations were carried out and numerical results of the subgrid model were compared both to those generated with no subgrid model and to fine-scale results of seven ice-wedge polygons. Our findings confirm that the subgrid model improves the shape of the hydrographs and the total water content in the system, and that the results are very close to the corresponding fine-scale simulations. The model also was applied to a 468 polygons catchment, and significant changes were observed in the results obtained with and without subgrid model. Our results indicate that the model has the potential  to accurately represent fine-scale behavior at larger spatial scales.
\end{abstract}
